...
int main(int argc, char *argv[]) {
    ...
    #pragma omp parallel default(shared) private(iam, np) firstprivate(rank)
    {
        ...
        if(iam==0 && rank==0){ 
        #pragma omp single nowait private(recebendo)
        { 
            MPI_Request request;
            MPI_Status status;
            int flag=0;
            printf("master, rank: %d, thread: %d\n", rank, iam);
            #pragma omp task depend (in:recebendo, flag, request, status)
            { 
                while(flag==0){
        ...
        if(iam==1 && rank==1){
        #pragma omp single nowait private(enviando)
        {
            
            char enviando[50] = "serah que chega? ";
            #pragma omp task depend (out:enviando)
            {  
...
//primeira tentativa: out:enviando ;  in:recebendo